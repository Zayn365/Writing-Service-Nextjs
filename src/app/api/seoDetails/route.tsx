...
import * as SeoDetails from "../../../../models/SeoDetails"


export async function GET(req: NextRequest) {
  try {
    const data = await SeoDetails.get();
...
export async function POST(req: Request, res: NextApiResponse) {
  try {
    ...
    const user = await SeoDetails.create(data);
...
export async function PUT(req: any, res: NextApiResponse) {
  try {
    ...
    const user = await SeoDetails.update(id as any, data); // Updating the user
...
export async function DELETE(req: any, res: NextApiResponse) {
  try {
    ...
    const searchParam = new URLSearchParams(url.searchParams);
    const id = searchParam.get('id');
    const user = await SeoDetails.Delete(id as any);
    return NextResponse.json({ message: "Operation Successfull", data: user }, { status: 201 });
  } catch (e: any) {
    return NextResponse.json({ error: e.message }, { status: 500 });
  }
